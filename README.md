...
Crucible is organized into two layers:

### Persistent Core
Always running. Provides the infrastructure that supports all research workflows.

| Component | Role |
|---|---|
| Elastic Stack (Elasticsearch + Kibana + Fleet) | Centralized telemetry, detection rules, dashboards |
| Analysis Workstation (REMnux-based) | Static and dynamic malware analysis tooling |
| Network Infrastructure | Isolated VLANs, simulated internet (INetSim/FakeNet-NG), DNS |
| Jupyter Notebook Server | Research documentation, Elastic API queries, artifact correlation |

### Ephemeral Targets
Spun up per research task, torn down when complete.

| Target Type | Examples |
|---|---|
| Windows Endpoint | Windows 10/11 workstation, Windows Server |
...
Terraform provisions VMs via the Proxmox provider. Full environment with all persistent core components and multiple concurrent ephemeral targets.
...
Provisions isolated VNets with no public internet egress from analysis or target VMs. Designed for cost-conscious use — destroy your environment when your research session ends.
...
1. Intake sample to the analysis workstation (isolated network segment)
...
4. Dynamic artifacts (process creation, network connections, file system changes, registry modifications) are automatically forwarded to Elastic
5. Correlate static and dynamic findings in Kibana or Jupyter Notebook
6. Export findings as structured data to Elastic custom index
...
3. Execute the technique (manually or via Atomic Red Team)
4. Observe telemetry in Elastic — what data sources fired, what fields are populated
5. Write a detection rule in Elastic Security
6. Validate the rule fires correctly, tune for false positive reduction
7. Document the technique, artifacts, and detection logic
...
- [ ] Persistent core provisioning (Elastic Stack, Fleet, Analysis Workstation)
- [ ] Ephemeral Windows target template
- [ ] Ephemeral Linux target template
- [ ] INetSim / FakeNet-NG simulated internet layer
- [ ] Jupyter Notebook integration with Elasticsearch Python client
- [ ] Atomic Red Team integration
- [ ] Ghidra to Elastic artifact export pipeline
- [ ] macOS target support
- [ ] Network OS target support
- [ ] Detection rule library (ATT&CK aligned)
